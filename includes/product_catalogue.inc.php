<?php
  ...
  const CATEGORY_NAMES = ["Clothing", "Accessories", "Shoes"];
  const BRAND_NAMES = ["GreyDay", "Nike", "Jordan"];
  ...
  function generateItemList($items){
    ...
    while ($itemIdx < $itemCount) {
      ...
      for ($i=0; $itemIdx < $itemCount && $i < 4; $itemIdx++){
        ...
        $price = "RON" . number_format($price, 2);
          ...
          <div class='col s3'>
            <a href='product.php?item_id=$itemID'>
              <div class='selectable-card' style='height: 480px; min-width: 300px'>
                <img class='shadow-img center' src='product_images/$image' style='height: 300px; max-width: 300px; 
                border-radius: 10px; margin: 0 auto; object-fit:scale-down; margin-top: 15px; margin-left: 17px;'>
